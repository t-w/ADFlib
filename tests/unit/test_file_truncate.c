...
void test_file_truncate( test_data_t * const tdata )
{
    ...
    /*unsigned expected_nDataBlock =
        ...
        pos2datablockIndex( (unsigned) max( (int) truncsize - 1, 0 ),
                            vol->datablockSize ) + 1;
    ...
    */
    ...
    if ( file->fileHdr->byteSize == 0 )
        ...
        const unsigned nonZeroExpected = ( nDataBlocks % ADF_MAX_DATABLK != 0 ?
    ...
    const unsigned commonSize = min( bufsize, truncsize );
    ...
    if ( truncsize > bufsize ) {
        ...
        const size_t extrasize = truncsize - bufsize;
        uint8_t * const ebuf   = malloc( extrasize );
...
Suite * adflib_suite( void )
{
    ...
    tcase = tcase_create( "adflib test_file_truncate_ofs" );
    tcase_add_test( tcase, test_file_truncate_ofs );
    tcase_set_timeout( tcase, 120 );
    suite_add_tcase( suite, tcase );

    tcase = tcase_create( "adflib test_file_truncate_ffs" );
    //tcase_add_checked_fixture ( tc, setup_ffs, teardown_ffs );
    tcase_add_test( tcase, test_file_truncate_ffs );
    tcase_set_timeout( tcase, 120 );
    suite_add_tcase( suite, tcase );
