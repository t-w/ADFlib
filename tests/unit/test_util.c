...
#ifdef _MSC_VER   // visual studio does not allow use const as array size
...
{
    const unsigned fsize = file->fileHdr->byteSize;

    const ADF_RETCODE rc = adfFileSeek( file, fsize + 1 );
    if ( rc != ADF_RC_OK || ! adfFileAtEOF( file ) ) {
        return 1;
    }
    //unsigned nerrors = 0;
    const unsigned nDataBlocks = adfFileSize2Datablocks( file->fileHdr->byteSize,
                                                         file->volume->datablockSize );
    const unsigned nExtBlocks  = adfFileDatablocks2Extblocks( nDataBlocks );
                                 ...
                                 const unsigned           errors_max )
{
    ...
    if ( ! file )
        return 1;
    const unsigned nerrors = validate_file_metadata_last_ext( file );
    adfFileClose( file );
    return nerrors;
}
                          ...
                          const unsigned blocksize )
{
    //assert ( fsize >= 0 );
    //assert ( blocksize >= 0 );
    const unsigned data_blocks = filesize2datablocks( fsize, blocksize );
    const unsigned ext_blocks  = datablocks2extblocks( data_blocks );
