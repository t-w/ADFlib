...
//    ck_assert_int_eq( 0, file->posInExtBlk );
    ...
    ck_assert_int_eq( adfFileAtEOF( file ), true );  // sure ? always 2nd part is bigger?
    adfFileClose( file );
...
Suite * adflib_suite( void )
{
    ...
    tcase = tcase_create( "adflib test_file_overwrite_ofs" );
    tcase_add_test( tcase, test_file_overwrite_ofs );
    tcase_set_timeout( tcase, 60 );
    suite_add_tcase( suite, tcase );

    tcase = tcase_create( "adflib test_file_overwrite_ffs" );
    //tcase_add_checked_fixture( tcase, setup_ffs, teardown_ffs );
    tcase_add_test( tcase, test_file_overwrite_ffs );
    tcase_set_timeout( tcase, 60 );
    suite_add_tcase( suite, tcase );
