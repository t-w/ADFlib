...
void test_file_write( test_data_t * const tdata )
{
    ...
    const unsigned free_blocks_before = adfCountFreeBlocks( vol );
    ck_assert_int_eq( tdata->nVolumeBlocks, free_blocks_before );
    const int nentries = adfDirCountEntries( vol, vol->curDirPtr );
...
*/
    ...
    for ( unsigned i = 0; i < nchunks; ++i )  {
        ...
        const unsigned wsize = ( i == nchunks - 1 ) ?   // the last chunk
        ...
        const unsigned bytes_written = adfFileWrite( file, wsize, chunk );
    ...
    //                  file->posInExtBlk );
    const unsigned expected_posInExtBlk = datablock2posInExtBlk(
        filesize2datablocks( bufsize, vol->datablockSize ) );
    ...
    const unsigned expected_free_blocks =
    ...
    unsigned char * const rbuf = malloc( bufsize );
    ck_assert_ptr_nonnull( rbuf );
    const unsigned bytes_read = adfFileRead( file, bufsize, rbuf );
    free( rbuf );
    ...
    //ck_assert_int_eq( 1, file->posInDataBlk );
    const unsigned expected_nDataBlock =
...
Suite * adflib_suite( void )
{
    ...
    tcase = tcase_create( "adflib test_file_write_ofs" );
    tcase_add_test( tcase, test_file_write_ofs );
    tcase_set_timeout( tcase, 120 );
    suite_add_tcase( suite, tcase );

    tcase = tcase_create( "adflib test_file_write_ffs" );
    //tcase_add_checked_fixture( tcase, setup_ffs, teardown_ffs );
    tcase_add_test( tcase, test_file_write_ffs );
    tcase_set_timeout( tcase, 120 );
    suite_add_tcase( suite, tcase );
